...
% "bottle-neck" neural network of ordinary stacked autoencoders with 3   % 
...
fprintf('--------------------------- Deep Bottleneck Network with SAE initialization --------------------------\n');
...
autoenc1 = trainAutoencoder(inputs, hiddenSize(1), ...     
                            'MaxEpochs', 600, ...
...
if (inputType == 1)
    ...
    totalMSE_SAE_training = estimateTotalMSE(data, reconstructions_training, mode);
    
    % time-lapse movie dataset
    if(dataType == 1)
   
        plotComparison(data, reconstructions_training, [1 5], [PCs_M hiddenSize PCs_M], reshapeVector, totalMSE_SAE_training);
...
% original/uncompressed inputs
    ...
    totalMSE_SAE_training = estimateTotalMSE(inputs, reconstructions_training, mode);
    
    % time-lapse movie dataset
    if(dataType == 1)
       
        plotComparison(inputs, reconstructions_training, [1 5], architectureVector, reshapeVector, totalMSE_SAE_training);
        
    else
        
        fprintf('--- mRNA gene expression average of relative reconstruction errors ---\n');
        fprintf('Bottleneck SAE initialization: %.5f\n', totalMSE_SAE_training);
...
if (dataType == 1)
    ...
    totalMSE_SAE_test = estimateTotalMSE(test_images_40, outputs, 'im');
    fprintf('Total MSE on the test dataset: %.5f', totalMSE_SAE_test);
    fprintf('\n');

    % visual inspection of first 5 original vs. reconstructed test images
    plotComparison(test_images_40, outputs, [1 5], architectureVector, reshapeVector, totalMSE_SAE_test);
